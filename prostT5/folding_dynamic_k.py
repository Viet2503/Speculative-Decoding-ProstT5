...
FIXED_KS = [1, 2, 3, 4, 5, 6, 8, 11, 15]
TAUS = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95]
CONFIGS = [("static", 0), ("prefix", 5)]   # (mode, prefix order p)
...
def main() -> int:
    ...
    for uid, q3, proj in loaded:
        ...
        drafter = FoldingProfileDrafter(proj[:N_HOMOLOGS], L, sym_to_idx)
...
def _headline(rows, n):
    print(f"=== Dynamic-K comparison ({n} proteins, drafter: prefix p=5, N={N_HOMOLOGS}) ===")
    pr = [r for r in rows if r["config"] == "prefix_p5"]
...
def _plot(path, rows, n):
    ...
    for ax, cfg, title in [(axes[0], "static", "static (prefix-blind)"),
                           (axes[1], "prefix_p5", "prefix-aware p=5")]:
