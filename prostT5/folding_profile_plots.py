...
def grid_figure(rdir, summ, n_prot) -> None:
    ...
    P_ORDER = ["static", 1, 2, 3, 4, 5]
    green = plt.cm.Greens

    def p_color(c): return "darkorange" if c == "static" else green(0.35 + 0.6 * (c - 1) / 4)
    def p_fmt(c):   return "o--" if c == "static" else "o-"
    def p_label(c): return "static" if c == "static" else f"p={c}"
    def n_color(n): return plt.cm.plasma(0.08 + 0.74 * N_VALUES.index(n) / 4)
    ...
    # [1,1] mean k vs K, one line per #homologs  (p=5)
    ax = axes[1, 1]
    for n in N_VALUES:
        s = cell("mean_mean_k", 5, "K", n_homologs=n)
        ax.plot(K_VALUES, [s.get(k) for k in K_VALUES], "o-", color=n_color(n), label=f"N={n}")
    ax.axhline(1.0, color="red", ls="--", alpha=0.4)
    ax.set_xlabel("Draft length K"); ax.set_ylabel("Mean k  (tokens / model call)")
    ax.set_title("Mean k vs K  (per #homologs, p=5)"); ax.legend(fontsize=8); ax.grid(True, alpha=0.3)
