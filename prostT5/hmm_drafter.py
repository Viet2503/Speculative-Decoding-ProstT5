...
class HMMDrafter:
    ...
    # ----- adapter for notebook spec_decode_greedy (Viet's interface) -----

    def get_draft_logits(self, pos: int, accepted_prefix=None, enc_hidden=None) -> torch.Tensor:
        """Return emission scores from absolute decoder position `pos` onward.

        Shape: (remaining, 20). The caller slices to its K and argmaxes over the
        last dim to get an AA class index in AA_ORDER='ACDEFGHIKLMNPQRSTVWY',
        which matches the CNN drafter's class order. Prefix-blind: matches the
        H1/H2 design (PrefixAwareHMMDrafter is the H5 variant).
        """
        if pos >= self.L:
            return self._E[self.L:self.L]
        return self._E[pos:self.L]
...
def build_hmm_drafters_for_test_set(
    test_set: dict,
    hmm_path: str | Path,
    tokenizer,
    aa_key: str = "aa",
    ...
    """Build one HMMDrafter per protein in `test_set`.

    `test_set` is `{uid: {"aa": str, ...}}`. Returns `{uid: HMMDrafter or None}`
    so the notebook can skip proteins where alignment failed.
    """
    drafters: dict = {}
    n_ok = 0
    for uid, rec in test_set.items():
        ...
        if not aa:
            drafters[uid] = None
            if verbose:
                print(f"  {uid}: no AA seq -> skip")
            continue
        try:
            drafters[uid] = HMMDrafter(hmm_path, aa, tokenizer)
            n_ok += 1
        except Exception as exc:
            drafters[uid] = None
            if verbose:
                print(f"  {uid}: build failed: {type(exc).__name__}: {exc}")
    if verbose:
        print(f"HMM coverage: {n_ok}/{len(test_set)}")
    return drafters
